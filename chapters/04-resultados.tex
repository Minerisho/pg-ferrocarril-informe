...
La Tabla \cref{tab:matriz-riesgos-final} presenta la matriz de riesgos, donde las columnas representan lo siguiente:\\
...
\begin{enumerate}
    \item \textbf{MainMenu:} Es la escena persistente que contiene el lienzo (\textit{Canvas}) principal de la interfaz y los gestores globales del sistema. Los objetos instanciados aquí están condos para no destruirse entre cargas (\textit{DontDestroyOnLoad}), asegurando la continuidad de la música, los datos del jugador y la gestión de estados.
...
Integrado dentro de las opciones del menú de pausa, se encuentra el módulo de consulta denominado \textbf{Guía} ( \cref{fig:guia_juego}). Este componente actúa como una enciclopedia interactiva y estática, diseñada para reforzar el aprendizaje de las mecánicas sin obligar al jugador a abandonar la experiencia.
