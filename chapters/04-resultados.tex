...
Adicionalmente, se configuró el uso de Git Large File Storage (LFS) para la gestión de prefabs y recursos de gran tamaño, lo cual permitió mantener un repositorio organizado y evitar problemas asociados al manejo de archivos pesados.
...
\begin{enumerate}
    \item \textbf{Facilidad de Aprendizaje (Learnability):} Vinculada a la \textit{HU29 (Tutorial)} \cite{608}. Se consultó al usuario si la guía de Don Raíl fue suficiente para entender las mecánicas de construcción sin necesidad de manuales externos.
    \item \textbf{Visibilidad del Estado del Sistema:} Basado en la heurística de mantener al usuario informado. Se evaluó si el HUD (presupuesto, vida del tren y alertas de daño) proporcionaba retroalimentación clara durante la simulación \cite{598}.
    \item \textbf{Prevención de Errores y Recuperación:} Se analizó la utilidad de la función de "Deshacer" (\textit{HU16}) y las advertencias contextuales de Don Raíl (\textit{HU31}) para evitar que el jugador diseñara rutas inviables \cite{609}.
...
[Aquí podrías insertar una gráfica de barras con los resultados generales]
...
Uno de los puntos más relevantes del análisis fue el abandono registrado en el \textbf{Nivel 3}. Al contrastar las respuestas de la encuesta con los metadatos (\textit{HU36}), se identificó que la complejidad de los costos de construcción en zonas de alta pendiente no era comunicada con suficiente antelación, rompiendo el "flujo" (\textit{Flow}) del jugador y generando frustración.
...
\begin{itemize}
    \item \textbf{Contexto Histórico:} El 75\% de los jugadores afirmó que las anécdotas de Don Raíl \cite{613} les ayudaron a comprender mejor la importancia de la infraestructura ferroviaria en Colombia.
