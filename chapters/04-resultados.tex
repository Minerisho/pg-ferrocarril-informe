...
\subsection{Recursos}
La gestión de recursos dentro del juego se centra en un presupuesto monetario virtual asignado al inicio de cada nivel. Este capital disminuye en tiempo real conforme el jugador construye tramos de vía, túneles o puentes, y al adquirir locomotoras y vagones. El diseño impone una restricción dura: si el saldo llega a cero antes de completar el objetivo, se impide la construcción de nuevos segmentos, obligando al jugador a optimizar el trazado o reiniciar el nivel.

\subsection{Zonas de daño y prohibición}
Para garantizar la coherencia geográfica y aumentar la dificultad, se implementaron volúmenes de control invisibles en el mapa. Las "zonas de prohibición" impiden la construcción de infraestructura en áreas reservadas (como cascos urbanos densos o reservas naturales), mientras que las "zonas de daño" detectan colisiones físicas del tren. Si el vehículo impacta contra obstáculos no previstos o se descarrila por exceso de velocidad en una curva, el sistema activa el estado de fallo del nivel.

\subsection{Managers}
La arquitectura del juego se orquesta a través de una serie de clases controladoras (\textit{Managers}) que centralizan la lógica de negocio y mantienen el estado global de la aplicación.

\subsubsection{Economía}
Este módulo es responsable de calcular el costo dinámico de la infraestructura. Aplica tarifas diferenciadas según el tipo de construcción (vía en superficie, estructura de puente o excavación de túnel) y gestiona las transacciones de compra de los trenes, actualizando la interfaz de usuario (UI) en cada fotograma.

\subsubsection{Sistema de Guardado}
Se diseñó un sistema de persistencia híbrido. La progresión local (niveles desbloqueados y configuración) se almacena mediante archivos binarios en el dispositivo del usuario, mientras que, como se mencionó anteriormente, la telemetría detallada de las sesiones se sincroniza con la base de datos en la nube.
...
Este componente opera en segundo plano recopilando datos críticos (clics, tiempo, presupuesto) y los empaqueta en objetos JSON. Al finalizar la partida, se encarga de la comunicación asíncrona con la API para el envío seguro de la información.

\subsubsection{Tutorial}
El sistema de tutorial no es una escena separada, sino una capa lógica superpuesta en el primer nivel. Utiliza disparadores (\textit{triggers}) espaciales y de eventos para pausar el juego momentáneamente y guiar al usuario mediante ayudas visuales y cuadros de texto, enseñando las mecánicas básicas de cámara y construcción de forma progresiva.

\subsection{Menú Principal}
El punto de entrada a la experiencia es una escena ligera diseñada para la navegación rápida. Desde aquí, el usuario puede acceder a la configuración, créditos y al inicio del juego. La interfaz, mostrada en la Figura \ref{fig:menu_principal}, mantiene la estética \textit{low-poly} del proyecto presentando una vista previa animada del entorno.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/menu_ferro.png}
    \caption{Interfaz del Menú Principal con el título y opciones de navegación.}
    \label{fig:menu_principal}
\end{figure}

\subsubsection{Guía}
Se incluye un módulo de referencia documental dentro del juego (Gladiolo), accesible desde el menú. Esta sección ofrece al jugador explicaciones estáticas sobre los controles y la simbología del juego, sirviendo como manual de usuario consultable en cualquier momento.
...
Cada nivel se presenta bajo la metáfora de un "Contrato Ferroviario". Antes de iniciar, el sistema presenta una ficha técnica (Figura \ref{fig:contrato}) que detalla el origen, el destino, el presupuesto asignado y una breve reseña histórica o ficticia que justifica la construcción de la vía, inmerso al jugador en el rol de ingeniero civil.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/contratoPuntuación.png}
    \caption{Ficha del contrato inicial que establece los objetivos y el presupuesto del nivel.}
    \label{fig:contrato}
\end{figure}
...
Finalmente, la puntuación numérica $P_{total}$ se traduce a un sistema visual de 1 a 3 estrellas... (continuar con el texto que ya tenías).

\subsubsection{Estrellas}
La puntuación numérica se traduce a un sistema visual de 1 a 4 estrellas para facilitar la lectura rápida del desempeño. Este cálculo se realiza comparando el puntaje final contra umbrales predefinidos en el \textit{Scriptable Object} del nivel. La Figura \ref{fig:victoria} muestra la pantalla de éxito donde se visualiza esta calificación junto con las opciones de navegación.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/pantalla_victoria.png}
    \caption{Pantalla de victoria mostrando la calificación por estrellas y el puntaje obtenido.}
    \label{fig:victoria}
\end{figure}\subsubsection{Estrellas}
