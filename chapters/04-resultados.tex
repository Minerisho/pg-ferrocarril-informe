...
\begin{equation}
    S_{remantente} = P_{inicial} - (C_{vias} + C_{estructuras} + C_{tren})
\end{equation}
...
\begin{itemize}
    \item \textbf{Estrella por Completado (Cumplimiento):} Es la distinción base que se otorga automáticamente al superar los requisitos mínimos del contrato, es decir, conectar el origen con el destino válido.
    ...
    \item \textbf{Estrella de Vida (Seguridad Operativa):} Premia la integridad de la maquinaria. Para conseguirla, el tren debe llegar a la meta sin haber sufrido ningún tipo de daño por colisiones o descarrilamientos parciales durante el trayecto.
    
    \item \textbf{Estrella de Tiempo (Eficiencia Temporal):} Se otorga al cumplir el objetivo de tiempo ideal estipulado en el contrato, lo cual exige una planificación de ruta óptima y una velocidad de conducción adecuada.
\end{itemize}

Finalmente, la Figura \ref{fig:victoria} muestra la pantalla de consolidación de éxito, donde se visualiza la sumatoria de estas calificaciones junto con el puntaje numérico global.
...
Para validar el prototipo como herramienta de aprendizaje y software funcional, se realizó una prueba piloto con \textbf{22 voluntarios} de la comunidad universitaria. El proceso de recolección de datos siguió un enfoque mixto: los datos \textbf{cuantitativos} fueron extraídos directamente de la API de informes (telemetría), mientras que los \textbf{cualitativos} se obtuvieron mediante una encuesta de percepción aplicada tras la sesión de juego.
...
La Figura \ref{fig:retencion} muestra la distribución de jugadores únicos por nivel. Se evidencia una deserción significativa tras el Nivel 1, donde la participación descendió de 28 a 9 jugadores (una caída del 67.8\%). 

Este fenómeno es común en videojuegos con mecánicas de construcción complejas. Los datos sugieren que el Nivel 1 actuó como un \textit{filtro} de aprendizaje donde los usuarios debieron dominar el sistema de curvas de Bézier y la gestión de presupuestos. La baja tasa de llegada al Nivel 5 (1 solo jugador) confirma que el videojuego no solo es una herramienta de visualización, sino un desafío técnico que requiere persistencia y una comprensión profunda de la lógica ferroviaria implementada.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/retencion.png} 
\caption{Tasa de retención de jugadores únicos por nivel.}
\label{fig:retencion}
\end{figure}

\subsubsection{Balance de Dificultad: Victorias frente a Derrotas}
Al analizar la relación entre éxitos y fracasos (Figura \ref{fig:tasa_exito}), resalta que el Nivel 1 posee una tasa de derrota del 71.4\%. Este dato, aunque elevado para una etapa inicial, se justifica por el carácter \textit{serio} del videojuego: el error es parte del proceso de aprendizaje. 

Se observa una mejora en el desempeño durante el Nivel 2 (66.7\% de éxito), indicando que quienes superaron la barrera inicial lograron una estabilización de sus habilidades. Sin embargo, el repunte de derrotas en los niveles avanzados (superando el 87\% en los niveles 4 y 5) valida que el diseño del mapa y las restricciones económicas representan fielmente la complejidad de la ingeniería civil en terrenos montañosos.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/tasa_exito.png} 
...
La percepción sobre los controles fue mayoritariamente positiva, con un 81.9\% de aprobación (Figura \ref{fig:facilidadcontroles}). No obstante, existe un 9\% de usuarios que reportaron dificultades, cifra que coincide con quienes calificaron su primera experiencia como confusa (Figura \ref{fig:primeraexperiencia}). Este cruce de datos sugiere que, aunque el sistema es intuitivo para la mayoría, el tutorial inicial es un punto crítico que define el compromiso del jugador a largo plazo.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/facilidadcontroles.png} 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/primeraexperiencia.png}
...
Esta sección constituye el núcleo de la validación pedagógica. Como indica la Figura \ref{fig:contruisviaslejanas}, un abrumador 81.8\% de los participantes concluyó que la construcción ferroviaria en Colombia es \textit{complicada y costosa}. Este resultado es fundamental: el videojuego logró transformar la percepción del usuario desde una visión simplista hacia una comprensión técnica de la realidad nacional.
...
Durante la prueba piloto, se reportaron los siguientes fallos que requieren corrección:
\begin{itemize}
    ...
    \item \textbf{Colisiones fantasma:} El tren detectó daño en el 2\% de las partidas sin colisión visual clara[cite: 131]. Se deben refinar los \textit{colliders}.
