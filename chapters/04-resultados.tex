...
La Figura \ref{fig:bd_nosql} ilustra la estructura de la colección principal denominada \textit{level\_logs}, la cual actúa como un registro histórico de cada intento realizado realizado en un nivel, sea de victoria o derrota. Este modelo utiliza una relación de composición para almacenar los datos de interacción espacial directamente dentro del documento de la sesión, optimizando así las consultas de lectura.
...
\begin{itemize}
    \item \textbf{sessionID (string):} Identificador único generado por Unity para diferenciar cada intento de juego, permitiendo trazabilidad individual incluso si un mismo jugador repite el nivel múltiples veces en diferentes sesiones.
    ...
    \item \textbf{status (string):} Describe el estado final de la partida. Puede tomar valores categóricos como \textit{Victoria} o \textit{Derrota} siendo fundamental para calcular tasas de éxito.
    
    \item \textbf{causeOfFailure (string):} Campo opcional que almacena la razón específica de la derrota (ej. \textit{Descarrilamiento}, \textit{Presupuesto Agotado}). Si el estado es de victoria, este campo permanece vacío.
    
    \item \textbf{budgetUsed (float):} Variable económica que registra la cantidad total de dinero virtual gastado por el jugador en la construcción de vías y compra de maquinaria tras terminar el intento.
...
\subsection{Sistema Ferroviario}
