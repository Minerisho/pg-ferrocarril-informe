...
En este capítulo se presentan los productos resultantes del proceso de desarrollo del videojuego serio, abarcando desde la consolidación de los requerimientos y el diseño de la arquitectura, hasta la implementación del prototipo y el análisis de los datos obtenidos durante la prueba piloto.

\section{Requerimientos Consolidados}

Como resultado de la fase de inicio y elaboración descrita en la metodología, se obtuvieron los artefactos finales que guiaron la construcción del software. A diferencia de las iteraciones iniciales, estos artefactos representan la versión refinada y aprobada de los requisitos funcionales y el análisis de riesgos.

\subsection{Matriz de Riesgos Final}

Durante la planificación, se identificaron y evaluaron los posibles obstáculos técnicos y de diseño. La Tabla \ref{tab:matriz-riesgos-final} presenta la matriz de riesgos definitiva, incluyendo el estado de cierre de cada riesgo al finalizar el desarrollo. Los niveles de riesgo se calcularon basándose en el impacto y la probabilidad de ocurrencia (Escala 1-5).

\begin{landscape}
\begin{longtable}{|c|L{2.8cm}|L{2.8cm}|c|c|c|L{3.2cm}|c|}
\caption{Matriz de riesgos final del proyecto}\label{tab:matriz-riesgos-final} \\
\hline
...
R2 & La cámara atraviesa el terreno & Falta de colisión en la cámara & 2 & 5 & 10 & Implementación de *Clamping* y colisionadores para la cámara. & Abierto \\ \hline
...
R4 & Terreno con parches o vacíos & DEM con datos erróneos & 2 & 3 & 6 & Corrección mediante algoritmos de suavizado de terreno. & Cerrado \\ \hline
R5 & Textura de vías solapada (Z-fighting) & Vía al mismo nivel del suelo & 1 & 2 & 2 & Offset vertical en la generación de la malla de la vía. & Cerrado \\ \hline
R6 & Circuito cerrado no detectado & Spline no configurado como 'Loop' & 2 & 3 & 6 & Detección de proximidad de nodos inicial/final para cerrar el ciclo. & Cerrado \\ \hline
R7 & Dificultad para representar hidrografía completa & Densidad excesiva de ríos en Colombia & 4 & 4 & 16 & Selección de ríos principales y lagos mayores únicamente. & Abierto \\ \hline
R8 & Las vías atraviesan montañas sin túnel & Falta de interpolación de alturas & 3 & 4 & 12 & Generación automática de túneles al detectar colisión con terreno. & Cerrado \\ \hline
...
R13 & Bajos FPS en equipos de gama baja & Desarrollo en equipos de gama alta & 5 & 3 & 15 & Optimización de *draw calls* y reducción de polígonos. & Abierto \\ \hline
\end{longtable}
\end{landscape}
...
8 & HU29 & Como nuevo jugador, quiero un tutorial interactivo con "Don Raíl". & S & 4 \\ \hline
...
La fase de implementación se llevó a cabo utilizando el motor Unity (versión 2022 LTS), haciendo uso de Git para el control de versiones y LFS para la gestión de activos pesados. A continuación, se detalla el proceso técnico de construcción del entorno y las mecánicas ferroviarias.
...
La fase de pruebas tuvo una duración de **4 días**, contando con la participación voluntaria de \textbf{[INSERTAR NÚMERO TOTAL]} estudiantes universitarios.
...
Se preguntó sobre la facilidad de uso de las herramientas de construcción. El [XX]\% de los encuestados calificó el sistema de splines como "Intuitivo" o "Muy Intuitivo". Sin embargo, un [XX]\% reportó dificultades con la cámara en dispositivos con pantallas pequeñas.
...
\begin{itemize}
    \item \textbf{Cámara en límites del mapa:} En ocasiones, la cámara permite visualizar el "vacío" si el jugador se desplaza a los bordes extremos del mapa. Se requiere implementar límites rígidos (*clamping*) en las coordenadas de la cámara.
    \item \textbf{Colisiones fantasma:} En el 2\% de las partidas registradas por la API, el tren detectó daño sin haber una colisión visual clara. Esto sugiere la necesidad de refinar los *colliders* de las zonas de riesgo.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Expansión del contenido histórico:} Incluir más narrativa sobre la historia real de los Ferrocarriles Nacionales de Colombia, desbloqueable mediante coleccionables en el mapa.
